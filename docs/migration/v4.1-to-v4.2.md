...
- Old `content_format` parameter renamed to `output_format` but deprecated alias provided
...
result = extract(
    "document.pdf",
    content_format="markdown"  # NEW: explicit format control
)
...
// Request Markdown formatted text
const result = await extract('document.pdf', {
    contentFormat: 'markdown'  // NEW: explicit format control
});
...
func main() {
    ...
    config.ContentFormat = "markdown"  // NEW: explicit format control
...
config.setContentFormat("markdown");  // NEW: explicit format control
...
result = extract(
    "document.pdf",
    content_format="markdown",    # Format text as Markdown
...
const result = await extract('document.pdf', {
    contentFormat: 'markdown',    // Format text as Markdown
...
result = extract(
    "document.pdf",
    content_format="djot"  # NEW: Djot format support
)
...
const result = await extract('document.pdf', {
    contentFormat: 'djot'  // NEW: Djot format support
});
...
|-------|------|---------|---------|-------------|
| `content_format` | string | `plain`, `markdown`, `djot`, `html` | `plain` | Control output text formatting |
...
- `text` - Element text content (formatted according to `content_format`)
...
### Q: Can I use both `content_format` and `result_format` together?

A: Yes! They work independently:
- `content_format` controls how the text looks (markdown, djot, etc.)
