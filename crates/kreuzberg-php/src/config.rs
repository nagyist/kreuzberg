...
fn php_array_to_hashmap(arr: &ZendHashTable) -> HashMap<String, &Zval> {
...
/// Extract string value from HashMap with default
fn get_string_or_default(data: &HashMap<String, Zval>, key: &str, default: String) -> String {
    data.get(key)
        .and_then(|v| v.string())
        .map(|s| s.to_string())
        .unwrap_or(default)
...
/// Extract i32 value from HashMap with default
fn get_i32_or_default(data: &HashMap<String, Zval>, key: &str, default: i32) -> i32 {
    data.get(key)
        .and_then(|v| v.long())
        .map(|l| l as i32)
        .unwrap_or(default)
...
fn get_usize_or_default(data: &HashMap<String, Zval>, key: &str, default: usize) -> usize {
    data.get(key)
        .and_then(|v| v.long())
        .map(|l| l as usize)
        .unwrap_or(default)
...
/// Extract f32 value from HashMap with default
fn get_f32_or_default(data: &HashMap<String, Zval>, key: &str, default: f32) -> f32 {
    data.get(key)
        .and_then(|v| v.double())
        .map(|d| d as f32)
        .unwrap_or(default)
